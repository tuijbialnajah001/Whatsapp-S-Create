...
import { 
  Film, Upload, Settings2, Download, AlertCircle, Loader2, ArrowRight, X, Play, Package
} from 'lucide-react';
...
export default function VideoStickerCreate() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter(f => f.size <= 25 * 1024 * 1024); // 25MB limit per file
    if (validFiles.length < files.length) {
      setGlobalError(`Some files were skipped because they exceed the 25MB limit.`);
    }
  ...
  const processAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < videos.length; i++) {
        ...
        setVideos(prev => prev.map(v => v.id === videoId ? { ...v, status: 'processing', progress: 0 } : v));

        // Listen for progress
        ffmpeg.on('progress', ({ progress }) => {
          setVideos(prev => prev.map(v => v.id === videoId && v.status === 'processing' 
        ...
        try {
          ...
          setVideos(prev => prev.map(v => v.id === videoId ? { ...v, status: 'done', progress: 100, resultUrl: url } : v));
      ...
      console.error(err);
      setGlobalError("Failed to load processing engine. Please reload the page and try again.");
    } finally {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6">
      <div className="text-center mb-10">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight mb-4 flex items-center justify-center gap-3">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto text-sm sm:text-base leading-relaxed">
          Select multiple videos/GIFs. We'll automatically convert them into WhatsApp-ready animated WebP stickers.
          Max 8 seconds, 24fps, exact 512x512 resolution.
        </p>
      </div>

      <div className="glass-panel rounded-[2rem] p-6 sm:p-8 bg-white/50 dark:bg-zinc-900/50">
        
        {globalError && (
          <div className="mb-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-2xl flex items-start gap-3 items-center">
        ...
        {videos.length === 0 ? (
          <div className="relative border-3 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl p-12 text-center hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group cursor-pointer">
            <input 
              type="file" 
              multiple
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">Select Videos</h3>
            <p className="text-zinc-500 text-sm">MP4, WebM, GIF (Max 25MB each)</p>
          </div>
        ) : (
          <div className="space-y-8">
            <div className="flex flex-wrap gap-4 items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-6">
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-medium text-zinc-500">{videos.length} videos selected</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                {doneCount > 0 && (
                  ...
                  >
                    ...
                    {doneCount > 1 ? `Download ${doneCount} Stickers (ZIP)` : 'Download Sticker'}
                  </button>
                )}
                ...
                {pendingCount > 0 && (
                  ...
                  >
                    {isConverting ? (
                      ...
                      <><Play className="w-4 h-4 fill-current"/> Convert {pendingCount} Videos</>
                    )}
                  </button>
                )}
              </div>
                    ...
                    <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
                      {video.status === 'done' && video.resultUrl ? (
                        <div className="absolute inset-0 bg-checkerboard z-0">
                          <img src={video.resultUrl} alt="Done" className="w-full h-full object-contain relative z-10" />
                        </div>
                      ) : (
                        <video 
                      ...
                      {video.status === 'pending' && (
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                          <span className="px-3 py-1 bg-black/60 backdrop-blur-md text-white text-xs font-bold rounded-full">Waiting</span>
                        </div>
                      )}
                      ...
                      {video.status === 'processing' && (
                        <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-4">
                          ...
                          <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                            <div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${video.progress}%` }} />
                          </div>
                          <span className="text-[10px] text-white mt-1 font-bold">{video.progress}%</span>
                        </div>
                      )}
                      ...
                      {video.status === 'error' && (
                        <div className="absolute inset-0 bg-red-900/60 flex flex-col items-center justify-center p-4">
                          ...
                          <span className="text-[10px] text-white font-bold text-center">Failed</span>
                        </div>
                      )}
                    </div>
                  </motion.div>
