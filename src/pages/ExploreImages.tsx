...
const ImageCard = React.memo(({ img, isSelected, onToggle }: any) => {
  ...
  return (
    ...
    >
      ...
      {/* Selection Overlay */}
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${isSelected ? "bg-emerald-500/20 opacity-100" : "bg-black/40 opacity-0 group-hover:opacity-100"}`}
      >
...
export default function ExploreImages() {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (!response.ok) {
          if (i === 0) {
            ...
            throw new Error(
              errorData.error || "Failed to fetch images from server.",
            );
          } else {
  ...
  const handleDownloadIndividually = async () => {
    ...
    for (let i = 0; i < selectedImgs.length; i++) {
      ...
      const safeSearchQuery = searchQuery
        ? searchQuery.replace(/[^a-z0-9\s]/gi, "_").trim()
        : "image";
    ...
    try {
      ...
      for (let i = 0; i < selectedImgs.length; i++) {
        ...
        const safeSearchQuery = searchQuery
          ...
          : "image";
        const trueUniqueId = Math.random().toString(36).substring(2, 8);
        const filename = `Explore_${safeSearchQuery}_${i + 1}_${trueUniqueId}.jpg`;
      ...
      console.error(err);
      setError("Failed to transfer images. Please try again.");
    } finally {
      setDownloadState("none");
    }
  };
  ...
  const handleDownloadAsZip = async () => {
    ...
    try {
      ...
      const safeSearchQuery = searchQuery
        ? searchQuery.replace(/[^a-zA-Z0-9]/g, "_").trim()
        : "images";
  ...
  return (
    ...
    >
      ...
      {images.length > 0 && !error && (
        <div className="flex flex-wrap flex-col sm:flex-row items-center justify-between gap-4 mb-6 bg-white/50 dark:bg-zinc-900/50 p-4 rounded-3xl backdrop-blur-md border border-zinc-200/50 dark:border-zinc-800/50 sticky top-24 z-30 shadow-sm">
          <div className="flex items-center gap-4 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0 hide-scrollbar">
            <button
              onClick={toggleSelectAll}
              className="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 hover:text-emerald-500 dark:hover:text-emerald-400 font-bold transition-colors whitespace-nowrap bg-zinc-100 dark:bg-zinc-800 px-4 py-2 rounded-full"
            >
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 w-full sm:w-auto justify-end mt-2 sm:mt-0">
            <button
              onClick={handleDownloadIndividually}
              disabled={selectedIds.size === 0 || downloadState !== "none"}
              className="flex-1 sm:flex-none justify-center flex items-center gap-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 px-4 py-2.5 rounded-full font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              <span className="hidden sm:inline">Download</span> Files
            </button>
            <button
              onClick={handleDownloadAsZip}
              disabled={selectedIds.size === 0 || downloadState !== "none"}
              className="flex-1 sm:flex-none justify-center flex items-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2.5 rounded-full font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg hover:-translate-y-0.5"
            >
              {downloadState === "zip" ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
            ...
            <button
              ...
              disabled={selectedIds.size === 0 || downloadState !== "none"}
              className="col-span-2 sm:col-span-1 flex-1 sm:flex-none justify-center flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-full font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg hover:-translate-y-0.5"
            >
              {downloadState === "sticker" ? (
